// thank you https://raw.githubusercontent.com/chriskrycho/v5.chriskrycho.com/main/types/eleventy.d.ts
...
export type EngineName = 'html' | 'md' | 'js' | '11ty.js' | 'liquid' | 'njk' | 'hbs' | 'mustache' | 'ejs' | 'haml' | 'pug' | 'jstl'
...
export interface Config {
  ...
  addCollection(name: string, builder: (collection: Collection) => Page[] | Record<string, unknown> | Promise<Record<string, unknown>>): void

  addFilter(name: string, filter: AnyFunction): string | void

  addTransform(name: string, transform: (content: string, outputPath: string) => string | Promise<string>): string

  addLinter(name: string, linter: (content: string, inputPath: string, outputPath: string) => void | Promise<void>): void

  addShortcode(name: string, shortcode: AnyFunction<string> | AnyFunction<Promise<string>>): string
  addAsyncShortcode(name: string, shortcode: AnyFunction<string> | AnyFunction<Promise<string>>): Promise<string>
  addLiquidShortcode(name: string, shortcode: AnyFunction<string> | AnyFunction<Promise<string>>): void
  addNunjucksShortcode(name: string, shortcode: AnyFunction<string>): void
  addNunjucksAsyncShortcode(name: string, shortcode: AnyFunction<Promise<string>>): void
